...
!! perform rotations in harmonic space.
!
!! @author D. J. Mortlock
!! @version 0.1 Compiled in s2 library by JDM September 2005
!
! Revisions:
!   September 2005 - Jason McEwen
!------------------------------------------------------------------------------
...
module s2_dl_mod
  ...
  contains
    ...
    subroutine s2_dl_beta_operator(dl, beta, l)

      real(kind = s2_dp), pointer :: dl(:, :)
      real(kind = s2_dp), intent(in) :: beta
      integer, intent(in) :: l
  
      ! Fill in the top quarter of the d-matrix.
  
      call s2_dl_beta_operator_core(dl, beta, l)
  
      ! Use its symmetry properties to fill in the rest of it.
  
      call s2_dl_beta_operator_fill(dl, l)
    ...
    subroutine s2_dl_beta_operator_core(dl, beta, l)
      real(kind = s2_dp), pointer :: dl(:, :)
      real(kind = s2_dp), intent(in) :: beta
      integer, intent(in) :: l
  
      real(kind = s2_dp) :: lambda, xllp1, xm, big, bigi, bigi2, c, s, omc, &
        lbig, expo, renorm, ratio, c2, t, si, ang, big_const
      real(kind = s2_dp), pointer :: cp(:) => null(), cpi(:) => null(), &
      ...
      log_first_row(1)=(2.d0*real(l,kind=s2_dp))*dlog(dabs(c2))
      sign(1)=1.d0
      do i=2, 2*l+1
        m=l+1-i
        ratio=dsqrt(real(l+m+1,kind=s2_dp)/real(l-m,kind=s2_dp))
      ...
      xllp1=real(l*(l+1),kind=s2_dp)
  
      ! Initialising coefficients cp(m)= cplus(l-m).
  
      do m=1,l
        xm=real(l-m,kind=s2_dp)
      ...
      do index= 2,l+1
        ...
        lambda=(real(l+1,kind=s2_dp)*omc-real(index,kind=s2_dp)+c)*si
        dl(index - lp1, 2 - lp1)=lambda*dl(index - lp1, 1 - lp1)*cpi(1)
        if(index.gt.2) then
          do m=2,index-1
            lambda=(real(l+1,kind=s2_dp)*omc &
              -real(index,kind=s2_dp)+real(m,kind=s2_dp)*c)/s
      ...
      do index= l+2,2*l
        dl(index - lp1, 1 - lp1)=1.d0
        lambda=(real(l+1,kind=s2_dp)*omc-real(index,kind=s2_dp)+c)/s
        dl(index - lp1, 2 - lp1)=lambda*dl(index - lp1, 1 - lp1)*cpi(1)
        if(index.lt.2*l) then
          do m=2,2*l-index+1
            lambda=(real(l+1,kind=s2_dp)*omc-real(index,kind=s2_dp) &
              +real(m,kind=s2_dp)*c)/s
            dl(index - lp1, m+1 - lp1)= &
              lambda*cpi(m)*dl(index - lp1, m - lp1)-cp2(m)&
              *dl(index - lp1, m-1 - lp1)
            if(dl(index - lp1, m+1 - lp1).gt.big) then
    ...
    subroutine s2_dl_beta_operator_fill(dl, l)
      real(kind = s2_dp), pointer :: dl(:, :)
      integer, intent(in) :: l
