...
program s2_skyerror
  ...
  character(len=S2_STRING_LEN) :: filename1, filename2
  character(len=S2_STRING_LEN) :: file_type_str = MAP_FILE
  type(s2_sky) :: sky1, sky2
  real(s2_sp) :: er
  ...
  ! Compute error.
  ! (Incorporate functionality later to compute other error norms.)
  er = s2_sky_error_twonorm(sky1, sky2)
!  write(*,*) 'Two-norm error between ', trim(filename1), &
!       ' and ', trim(filename2), ' = ', er
  write(*,*) er, ';'
    ...
    subroutine parse_options()
      ...
      do i = 1,n,2
        ...
        select case (trim(opt))
          ...
          case ('-help')
            write(*,'(a)') 'Usage: s2_skyerror [-sky1 filename1]'
